...
module Data.Array.Accelerate.Trafo.NewFusion.LetBind
  ( letBindAcc
  )
where
...
letBindAcc (AST.OpenAcc pacc) = LabelledOpenAcc <$> case pacc of
  ...
  AST.Avar (ArrayVar idx) ->
    (`Variable` (ArrayVarsArray $ ArrayVar idx)) <$> getInc
  AST.Apair acc1 acc2 -> letBind acc1 $ \w1 var1 ->
    letBind (weaken w1 acc2) $ \w2 var2 ->
      Variable <$> getInc <*> (ArrayVarsPair <$> w2 $:> var1 <*> var2)
  AST.Anil -> (`Variable` ArrayVarsNil) <$> getInc
  AST.Apply f acc ->
    letBind acc $ \w var -> Apply <$> getInc <*> w $:> letBindAfun f <*> var
  AST.Aforeign asm fun acc -> letBind acc
    $ \_ var -> Aforeign <$> getInc <*> return asm <*> letBindAfun fun <*> var
  AST.Acond e left right ->
    Acond <$> getInc <*> letBindExp e <*> letBindAcc left <*> letBindAcc right
...
-- TODO do these need to exist?
  ...
  AST.Map f acc ->
    letBind acc $ \w var -> Map <$> getInc <*> w $:> letBindFun f <*> var
  AST.ZipWith f acc1 acc2 -> letBind acc1 $ \w1 var1 ->
    letBind (weaken w1 acc2) $ \w2 var2 ->
  ...
  AST.Fold1 f acc ->
    letBind acc $ \w var -> Fold1 <$> getInc <*> w $:> letBindFun f <*> var
  AST.FoldSeg f e acc seg -> letBind acc $ \w1 var1 ->
    letBind (weaken w1 seg) $ \w2 var2 ->
      FoldSeg
        <$> getInc
        <*> w2
        .   w1
        ...
        $:> var1
        <*> var2
  AST.Fold1Seg f acc seg -> letBind acc $ \w1 var1 ->
    letBind (weaken w1 seg) $ \w2 var2 ->
  ...
  AST.Scanl' f e acc -> letBind acc $ \w var ->
    Scanl' <$> getInc <*> w $:> letBindFun f <*> w $:> letBindExp e <*> var
  AST.Scanl1 f acc ->
    letBind acc $ \w var -> Scanl1 <$> getInc <*> w $:> letBindFun f <*> var
  AST.Scanr f e acc -> letBind acc $ \w var ->
    Scanr <$> getInc <*> w $:> letBindFun f <*> w $:> letBindExp e <*> var
  AST.Scanr' f e acc -> letBind acc $ \w var ->
    Scanr' <$> getInc <*> w $:> letBindFun f <*> w $:> letBindExp e <*> var
  AST.Scanr1 f acc ->
    letBind acc $ \w var -> Scanr1 <$> getInc <*> w $:> letBindFun f <*> var
  AST.Permute f acc1 g acc2 -> letBind acc1 $ \w1 var1 ->
    letBind (weaken w1 acc2) $ \w2 var2 ->
      Permute
        <$> getInc
        <*> w2
  ...
  AST.Stencil f b acc -> letBind acc $ \w var ->
    Stencil
      ...
      $:> letBindBoundary b
      <*> var
  AST.Stencil2 f b1 acc1 b2 acc2 -> letBind acc1 $ \w1 var1 ->
    letBind (weaken w1 acc2) $ \w2 var2 ->
      Stencil2
        <$> getInc
        <*> w2
...
letBindAfun
  ...
  -> State Int (PreOpenAfun LabelledOpenAcc aenv a)
...
makeLHSBV = makeLHSBV' . arraysRepr where
  ...
  makeLHSBV' a = case a of
    ArraysRunit ->
      Exists $ LHSBV (LeftHandSideWildcard ArraysRunit) ArrayVarsNil
    ArraysRarray ->
      Exists $ LHSBV LeftHandSideArray (ArrayVarsArray $ ArrayVar ZeroIdx)
    ArraysRpair left right -> case makeLHSBV' left of
      Exists (LHSBV leftlhs leftvar) -> case makeLHSBV' right of
        Exists (LHSBV rightlhs rightvar) -> Exists $ LHSBV
          (LeftHandSidePair leftlhs rightlhs)
          (ArrayVarsPair (weakenWithLHS rightlhs `weaken` leftvar) rightvar)
...
letBindExp openexp = case openexp of
 ...
 where
  letBindTup
    ...
    -> State Int (Tuple (LabelledOpenExp env aenv) t)
...
letBindBoundary
  :: PreBoundary OpenAcc env a -> State Int (PreBoundary LabelledOpenAcc env a)
